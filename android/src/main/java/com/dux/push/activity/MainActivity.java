...
public class MainActivity extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(com.dux.push.R.layout.activity_main);

    Intent intent = getIntent();

    //遍历所有自定义参数
    String url = intent.getDataString();
    Log.d("rnduxpush", " 回调= " + url);
    Map map = getUrlParams(url);
    Log.d("rnduxpush", " 自定义参数= " + map);
    Gson gson = new Gson();
    RNDuxPushPackage.reactContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class).emit("duxpush_click", gson.toJson(map));
    finish();
  }
